...
# __name__ is where the blueprint defined
    ...
    if request.method == 'POST':
        ...
        if error is None:
            try:
                ...
                # for any user input, and a tuple of values 
...
def login_required(view):
    @functools.wrap(view) # returns a new view function that wraps the original view it’s applied to
    def wrapped_view(**kwargs):
        if g.user is None:
            return redirect(url_for('auth.login')) # add to the 'auth' blueprints
        return view(**kwargs)
    return wrapped_view